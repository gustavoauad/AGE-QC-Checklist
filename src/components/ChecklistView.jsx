...
export default function ChecklistView({ project, userRole, session, onBack, onSignOut }) {
  ...
  // milestone mode
  const [viewMode, setViewMode] = useState("category"); // "category" | "milestone"
  const [milestones, setMilestones] = useState([]);
  const [activeMilestoneId, setActiveMilestoneId] = useState(null);
  const [milestoneItemsCache, setMilestoneItemsCache] = useState({}); // milestoneId -> Set<checklistItemId>
  ...
  const handleViewModeToggle = async (mode) => {
    setViewMode(mode);
    if (mode === "milestone" && activeMilestoneId) {
      await fetchMilestoneItems(activeMilestoneId);
    }
  };
  ...
  useEffect(() => {
    if (!activeCategory && enabledCategories.length > 0) {
  ...
  const fetchComments = async (itemId) => {
    ...
    const { data } = await supabase.from("checklist_comments")
      .select("*").eq("checklist_item_id", itemId).order("created_at");
  ...
  // Items to display in the current view
  ...
  const milestoneItems = activeMilestoneItemIds
    ? checklists.filter((c) => activeMilestoneItemIds.has(c.id))
    : [];
  ...
  const renderItem = (item, idx, totalInGroup) => {
    ...
    return (
      <div key={item.id} style={{
        ...
        <div style={{ padding: "14px 16px", display: "flex", alignItems: "flex-start", gap: "16px" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", gap: "12px", marginTop: "4px", flexWrap: "wrap", alignItems: "center" }}>
              {status === "complete" && completedByName && (
                <span style={{ fontSize: "11px", color: "#4ade80" }}>✓ {completedByName} · {formatDate(item.completed_at)}</span>
              )}
            </div>
          </div>
          <button onClick={() => toggleComments(item.id)} style={{
            flexShrink: 0, background: isCommentsOpen ? "#1e3a5f" : "transparent",
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0f172a", fontFamily: "Inter, sans-serif", display: "flex", flexDirection: "column" }}>
      ...
      {/* View mode toggle */}
      <div style={{ background: "#1e293b", borderBottom: "1px solid #334155", padding: "0 24px", display: "flex", alignItems: "center", gap: "4px" }}>
        {[
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <div style={{ width: "220px", background: "#1e293b", borderRight: "1px solid #334155", overflowY: "auto", padding: "12px" }}>
          {viewMode === "category" ? (
            ...
            milestones.length === 0 ? (
              <p style={{ color: "#64748b", fontSize: "12px", padding: "8px" }}>No milestones set up. Use Project Setup to add milestones.</p>
            ) : (
              milestones.map((m) => {
            ...
            <>
              {milestones.length === 0 ? (
                <div style={{ textAlign: "center", padding: "60px 0" }}>
                  ...
                  <p style={{ color: "#64748b", fontSize: "14px" }}>Go to Project Setup → Milestones to create milestones and assign checklist items.</p>
                </div>
              ) : !activeMilestoneId ? null : milestoneLoading ? (
                <p style={{ color: "#94a3b8" }}>Loading milestone items...</p>
              ) : activeMilestoneItemIds === null ? (
                ...
                <>
                  {(() => {
                    ...
                    return (
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
                        <div>
                          <h2 style={{ color: "#f1f5f9", margin: "0 0 4px", fontSize: "20px" }}>{m?.name}</h2>
                          <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                            <span style={{ color: "#94a3b8", fontSize: "13px" }}>
                              {m && new Date(m.date + "T00:00:00").toLocaleDateString()}
                            </span>
                            {isAlert && (
                              <span style={{ fontSize: "12px", color: "#f59e0b", background: "#451a03", padding: "2px 10px", borderRadius: "20px" }}>
