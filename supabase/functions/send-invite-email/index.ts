...
serve(async (req) => {
  ...
  try {
    ...
    const html = isNewUser ? `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 32px; background: #f8fafc;">
        <div style="background: #1e293b; border-radius: 12px; padding: 32px; color: #f1f5f9;">
          <h1 style="color: #0095da; margin: 0 0 8px;">AG&amp;E QAQC Checklist</h1>
          <hr style="border: none; border-top: 1px solid #334155; margin: 16px 0;" />
          <p style="font-size: 16px; margin: 0 0 16px;">Hi there,</p>
          <p style="margin: 0 0 16px;"><strong>${inviterName}</strong> invited you to join the project <strong>"${projectName}"</strong> as <strong>${roleLabel}</strong> on AG&amp;E QAQC Checklist.</p>
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "AG&E QAQC Checklist <onboarding@resend.dev>",
        to: [to],
        subject: `You've been added to "${projectName}" on AG&E QAQC Checklist`,
        html,
      }),
    });
